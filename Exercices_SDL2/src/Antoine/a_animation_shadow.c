...
void play_with_texture_3(SDL_Texture* my_texture,
  ...
  int nb_it = 200, i;                                // Nombre d'images de l'animation
    ...
    destination.y = 
        h * (1 - exp(-5.0 * i / nb_it) / 2 *
                     (1 + cos(10.0 * i / nb_it * 2 *
                              3.14)));            // hauteur en fonction du numéro d'image
...
void play_with_texture_5(SDL_Texture *bg_texture,
  ...
  i = 0;
  for (cpt = 0; cpt < nb_images_animation-2 ; ++cpt) {
  ...
  for(cpt = 0; cpt < 3; ++cpt) {
    ...
    SDL_RenderCopy(renderer, my_texture, &state[3+cpt%2], &destination);
  ...
  i = 4;
  for (cpt = 0; cpt < nb_images_animation - 4; ++cpt) {
    play_with_texture_1(bg_texture,         // identique à play_with_texture_1, où on a enlevé l'affichage et la pause
                          window, renderer); 
    SDL_RenderCopy(renderer,                  // Préparation de l'affichage
                   my_texture, &state[i], &destination);
...
int main(int argc, char** argv) {
  ...
  SDL_Texture* my_texture = NULL, *bg_texture = NULL;         // La texture
