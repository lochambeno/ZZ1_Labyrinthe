...
//complexite : o(log(n))
int inserer_tas(tas_t * tas, data_t valeur){
...
int min_tas(tas_t tas, data_t el1, data_t el2){
	int min = el1;

	if(tas.tas[el2] < tas.tas[el1]){
		min = el2;
	}
		...
		do{
			...
			if(enfant_gauche<tas->nbr_elements){
				...
				if(enfant_droite<tas->nbr_elements){
					...
					if(tas->tas[a_echange] < tas->tas[a_traite]){
				...
				//a traite s'il est plus petit que celui-ci
					...
					if(tas->tas[enfant_gauche] < tas->tas[a_traite]){
